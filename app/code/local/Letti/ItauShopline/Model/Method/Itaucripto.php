...
class Itaucripto
{
	...
	{
		// Obtém tamanho da string passado por parâmetro
		$i1 = strlen($s);
		 
		// Percorre 256 posições
		for($j = 0; $j <= 255; $j++)
		{
			// Obtém parte da string, partindo de 1 até o resto da
			// divisão de $j por $il (tamanho da string)
			// e retorna o primeiro caracter ASCII dessa string
			$this->key[$j] = ord(substr($s, $j % $i1, 1));
			// Preenche posição com o número da posição
			$this->sbox[$j] = $j;
		}
